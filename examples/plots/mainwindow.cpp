...
void MainWindow::setupDemo(int demoIndex)
{
  switch (demoIndex)
  {
    ...
    case 21: setupVector3Demo(ui->customPlot); break;
    case 22: setupTrajectoryDemo(ui->customPlot); break;
  }
...
void MainWindow::setupTrajectoryDemo(QCustomPlot *customPlot)
{
  ...
  map_ticker = QSharedPointer<QCPMapAxisTickerFixed>::create(ui->customPlot->xAxis, ui->customPlot->yAxis);
  map_ticker->setTickStep(10.0);
  map_ticker->setScaleStrategy(QCPAxisTickerFixed::ssNone);

  foreach (QCPAxis *axis, ui->customPlot->axisRect()->axes())
  {
...
void MainWindow::setupVector3Demo(QCustomPlot *customPlot)
{
  demoName = "Vector3 Demo";
  ...
  QCPAxisRect *middleAxisRect = new QCPAxisRect(customPlot);
  ...
  customPlot->plotLayout()->addElement(1, 0, middleAxisRect);
  customPlot->plotLayout()->addElement(2, 0, bottomAxisRect);
  // 默认缩放y轴
  ...
  // 共x轴
  connect(bottomAxisRect->axis(QCPAxis::atBottom), SIGNAL(rangeChanged(QCPRange)), middleAxisRect->axis(QCPAxis::atBottom), SLOT(setRange(QCPRange)));
  connect(middleAxisRect->axis(QCPAxis::atBottom), SIGNAL(rangeChanged(QCPRange)), topAxisRect->axis(QCPAxis::atBottom), SLOT(setRange(QCPRange)));
  connect(topAxisRect->axis(QCPAxis::atBottom), SIGNAL(rangeChanged(QCPRange)), bottomAxisRect->axis(QCPAxis::atBottom), SLOT(setRange(QCPRange)));
  // y轴label在axis内侧
  ...
  // QCPAxisRect *middleAxisRect = new QCPAxisRect(customPlot);
  // QCPAxisRect *bottomAxisRect = new QCPAxisRect(customPlot);
  // subLayout->addElement(0, 0, middleAxisRect);
  // subLayout->addElement(1, 0, bottomAxisRect);
  // subLayout->setColumnStretchFactor(0, 1); // left axis rect shall have 60% of width
  // subLayout->setColumnStretchFactor(1, 1); // right one only 40% (3:2 = 60:40)
  ...
  // 实现子图的动态显示
  // topAxisRect->setVisible(false);
  // customPlot->plotLayout()->take(topAxisRect);
...
void MainWindow::ShowSubplot(int const index)
{
  auto rect_show = ui->customPlot->plotLayout()->take(all_rects_[index]);
  // qDebug() <<  "ui->customPlot->plotLayout()->rowCount() = " << ui->customPlot->plotLayout()->rowCount();
  if (rect_show)
  {
  ...
  else
  {
    ...
    ui->customPlot->plotLayout()->insertRow(current_index);
    ui->customPlot->plotLayout()->addElement(current_index, 0, all_rects_[index]);
  }
  // for (auto [key, rect] : all_rects_)
  // {
  //   qDebug() << "rect" << key << " " << rect->visible();
  // }
  // qDebug() <<  "end rowCount() = " << ui->customPlot->plotLayout()->rowCount();
